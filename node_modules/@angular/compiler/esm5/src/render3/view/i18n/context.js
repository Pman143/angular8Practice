...
var I18nContext = /** @class */ (function () {
  ...
  I18nContext.prototype.appendBoundText = function (node) {
    ...
    phs.forEach(function (values, key) {
      return updatePlaceholderMap.apply(void 0, tslib_1.__spread([_this.placeholders, key], values));
    });
  };
...
function findTemplateFn(ctx, templateIndex) {
  return function (token) {
    return typeof token === 'object' && token.type === TagType.TEMPLATE &&
      token.index === templateIndex && token.ctx === ctx;
  };
}

function serializePlaceholderValue(value) {
  var element = function (data, closed) {
    return wrapTag('#', data, closed);
  };
  var template = function (data, closed) {
    return wrapTag('*', data, closed);
  };
  switch (value.type) {
